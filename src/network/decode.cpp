...
static flecs::entity mirror(flecs::world& world, NetworkConnection& conn, uint64_t nid) {
...
static void apply_components(flecs::world& world, const NetworkRegistry& reg, NetworkConnection& conn,
                             flecs::entity e, const std::vector<MessageComponentData>& comps, uint64_t tick) {
    bool got_pos = false, got_rot = false, is_bullet = false;
    glm::vec2 pos{0};
    float rot = 0;
    ...
    for (const auto& cb : comps) {
        ...
        if (!c || !e.is_alive()) continue;
        Reader cr(cb.bytes.data(), cb.bytes.size());
        if (c->name == "Position") { Position t{}; reg.decode(*c, &t, cr); pos = t.value; got_pos = true; }
        else if (c->name == "Rotation") { Rotation t{}; reg.decode(*c, &t, cr); rot = t.angle; got_rot = true; }
        else { if (c->name == "Bullet") is_bullet = true; reg.read(world, e, *c, cr); }
    }
    ...
    if (e.is_alive() && (got_pos || got_rot)) {
        ...
        if (got_pos || ready) {
            ...
            if (!e.has<Position>() && (got_pos || has_interp)) e.set<Position>({p});
            if (!e.has<Rotation>() && (got_rot || has_interp)) e.set<Rotation>({a});
            if (has_interp) e.get_mut<Interpolation>().push(p, a, static_cast<double>(tick));
        }
    }
    if (is_bullet && e.is_alive()) e.set<VelocityLinear>({bvel});
}

static void apply_welcome(flecs::world& world, NetworkConnection& conn, Reader& r) {
    MessageWelcome msg = util::decode<MessageWelcome>(r);
    ...
    SDL_Log("network: welcome peer=%u self=%llu components=%zu",
            conn.peer_id, (unsigned long long)conn.self, world.get<NetworkRegistry>().components.size());
}

static bool peek_is_bullet(const NetworkRegistry& reg, NetworkConnection& conn, const std::vector<MessageComponentData>& comps) {
    for (const auto& cb : comps) {
        auto rit = conn.remap.find(cb.server_id);
        const NetworkRegistry::Component* c = (rit != conn.remap.end()) ? reg.find(rit->second) : nullptr;
        if (c && c->name == "Bullet") return true;
    }
    return false;
}
...
static void apply_snapshot(flecs::world& world, NetworkConnection& conn, Reader& r) {
    ...
    MessageSnapshot snap = util::decode<MessageSnapshot>(r);
    ...
    if (conn.newest != 0 && tick > conn.newest) {
        ...
        double expected = static_cast<double>(tick - conn.newest);
        conn.jitter = conn.jitter * 0.9 + std::abs(arrived - expected) * 0.1;
    }
    if (tick >= conn.newest) { conn.newest = tick; conn.newest_time = t; }
    ...
    cmds.erase(std::remove_if(cmds.begin(), cmds.end(),
               [&](const Command& c) { return c.tick <= ack; }), cmds.end());
...
static void apply_structural(flecs::world& world, NetworkConnection& conn, Reader& r) {
    const auto& reg = world.get<NetworkRegistry>();
    MessageStructural s = util::decode<MessageStructural>(r);
    uint64_t tick = s.tick;

    for (const auto& sp : s.spawns) {
        uint64_t nid = sp.network_id;
        bool fresh = conn.entities.count(nid) == 0;
        ...
        if (newBullet && e.is_alive()) e.add<Latent>();
        auto& dq = conn.despawn_queue;
        dq.erase(std::remove_if(dq.begin(), dq.end(), [&](auto& d) { return d.first == nid; }), dq.end());
    }
    ...
    for (uint64_t nid : s.despawns) {
        ...
        if (eit != conn.entities.end()) {
            ...
            if (e.is_alive() && e.has<Dying>()) e.set<Dying>({UINT64_MAX});
        }
    }
}
...
void apply_packet(flecs::world& world, NetworkConnection& conn, ENetPacket* packet) {
    ...
    case Message::Welcome:    apply_welcome(world, conn, r); break;
    case Message::Snapshot:   if (conn.welcomed) apply_snapshot(world, conn, r); break;
    case Message::Structural: if (conn.welcomed) apply_structural(world, conn, r); break;
    default: break;
    }
}
