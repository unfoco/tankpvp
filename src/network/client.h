...
struct Sample { glm::vec2 position{0}; float angle = 0; double tick = 0; };
...
struct Interpolation {
    ...
    void push(glm::vec2 p, float a, double t) {
        ...
        head = (head + 1) % CAP;
        if (count < CAP) count++;
        position = p; angle = a; tick = static_cast<uint64_t>(t); ready = true;
    }
    const Sample& at(int i) const { return ring[(head - count + i + 2 * CAP) % CAP]; }
};

struct Command { uint64_t tick = 0; uint32_t flags = 0; uint32_t prediction = 0; uint32_t view = 0; double sent = 0; glm::vec2 muzzle{}; float aim = 0; };
...
struct NetworkConnection {
    ...
    struct Claim { uint32_t prediction = 0; uint64_t target = 0; int sends = 0; };
...
struct NetworkClient {
    NetworkClient(flecs::world&);
    static void teardown(flecs::world&);
private:
    static void pump(flecs::iter&);
    static void predict(flecs::iter&);
    static void interpolate(flecs::iter&);
    static void upload(flecs::iter&);
};
