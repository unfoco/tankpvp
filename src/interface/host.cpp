...
Clay_RenderCommandArray Interface::host(flecs::iter& it, InterfaceState& state, InterfacePage& page, InterfacePrevious& prev, const WindowEvents& events) {
    auto& target = it.world().get_mut<NetworkTarget>();

    Clay_BeginLayout();

    CLAY({
        .id = CLAY_ID("HostContainer"),
        .layout = {
        ...
        CLAY({
            ...
            CLAY({
                ...
                ButtonStyle hostBtn = { .color = { 70, 130, 255, 255 } };
