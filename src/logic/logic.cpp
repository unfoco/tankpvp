...
Logic::Logic(flecs::world& world) {
    ...
    world.system<Camera>("logic::camera_decay").kind(flecs::OnUpdate).each(Logic::camera_decay);
    world.system<PostStack>("logic::flash_decay").kind(flecs::OnUpdate).each(Logic::flash_decay);
}
