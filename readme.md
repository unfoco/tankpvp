...
**A server-authoritative multiplayer 2D game engine where games are Luau mods, not hardcoded rules.**
...
![tankpvp running CS-mode](asset/image/preview.png)
...
The engine spawns no player avatar. The mod declares the game-facing shape and hands the player control, and the engine fills in input, physics, the firing clock, lag-comp history, ownership, and replication automatically:
